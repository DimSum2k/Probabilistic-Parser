...
parser.add_argument('-sentence','--sentence', type=str, default="False",
...
size_test=round(len(corpus)*(1/10))
corpus_train=corpus[:(len(corpus)-2*size_test)]
corpus_test=corpus[(len(corpus)-size_test):]
corpus_dev=corpus[(len(corpus)-2*size_test):(len(corpus)-size_test)]
...
if str2bool(parser.sentence) == False:
	...
	pred_sent=[]
	print("parsing sentences in the file provided")
	...
	with open('parsed_sentences_from_provided_file.txt','w') as f:
		for line in pred_sent:
			f.write(line[2:][:-1])
			f.write("\n")
	print(" parsed sentences are in the file parsed_sentences_from_provided_file.txt")
